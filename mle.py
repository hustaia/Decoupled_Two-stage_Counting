...
def compute_mle(pred,gt):
    ...
    for i_pred_p in range(pred_num):
        ...
        match_matrix[i_pred_p,:] = pred_dist<=16

    tp, assign = hungarian(match_matrix)
    mle += np.multiply(assign,dist_matrix).sum()

    mle += 16*(pred_num+gt_num-2*tp)
    mle /= float(pred_num+gt_num-tp+1e-6)
